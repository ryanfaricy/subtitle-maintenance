    ...
    if tokens:
        ...
        if season >= 1900:
            ...
            candidates = [
                e
                for e in episodes
                if e.get("seasonNumber") == season and e.get("episodeNumber") == number
            ]
    elif date_numbering:
...
def recordings(root):
    ...
    for base, dirs, names in os.walk(root, onerror=fail, followlinks=False):
        dirs[:] = [
            ...
            for d in dirs
            if not d.startswith(".")
            and not d.lower().startswith(("transcod", "recording", "temporary"))
...
class API:
    ...
    def request(self, route, params=None, body=None):
        ...
        try:
            ...
            raise RuntimeError(
                "Sonarr HTTP %s: %s" % (exc.code, exc.read().decode()[:1200])
            ) from None
...
def main(argv=None):
    ...
    parser.add_argument(
        "--series", action="append", required=True, help="Exact Sonarr title; repeatable"
    )
    parser.add_argument("--year-numbered-series", action="append", default=[])
    parser.add_argument(
        "--date-numbered-series",
        action="append",
        default=[],
        help="Only for verified ORIGINAL air-date filenames",
    )
    ...
    try:
        settings = load_config(args.config).get("sonarr", {})
    ...
    for option in ("host_root", "sonarr_root", "work_dir"):
        ...
        if not value:
            parser.error(
                "Supply --" + option.replace("_", "-") + " or sonarr." + option + " in config"
            )
...
def run(args, api):
    ...
    def record(data):
        with journal.open("a") as stream:
            stream.write(
                json.dumps(
                    dict(time=datetime.datetime.now(datetime.timezone.utc).isoformat(), **data)
                )
                + "\n"
            )
            stream.flush()
        ...
        for path in recordings(root):
            ...
            if remote in registered:
                recovery = [
                    e
                    for e in episodes
                    if e.get("episodeFileId") == registered[remote]["id"]
                ...
                year_rename = (
                    episode and year_rule and bool(re.search(r"(?i)\bS\d{4}E\d+\b", path.name))
                )
            ...
            try:
                ...
                if existing_id:
                    ...
                    if existing.get("path") != remote:
                        counts["review"] += 1
                        print(
                            "REVIEW: Sonarr already has episode; keeping source:",
                            path.name,
                            flush=True,
                        )
                        continue
                    ...
                    if folder not in folder_items:
                        ...
                        folder_items[folder] = api.request(
                            "/manualimport", {"folder": folder, "filterExistingFiles": "false"}
                        )
                ...
                print(
                    ("APPLY " if args.apply else "WOULD ")
                    + action
                    + ": "
                    + path.name
                    + " -> "
                ...
                if previous.get("status") == "pending":
                    if previous.get("size") != before[2]:
                        raise RuntimeError(
                            "Pending import file size changed; manual review required"
                        )
