...
def load_config(filename=None):
    ...
    for section, keys in ((config, ('service_url',)),
    ...
    for section, keys in ((config, ('python', 'model')), (config.get('tools', {}), tuple(config.get('tools', {})))):
        for key in keys:
...
def tool(config, name):
    ...
    if not found:
        raise ValueError(f'{name} executable not found; install it on PATH or set tools.{name} in config')
    return found
