...
def phrase_evidence(cues, words, size=4):
    ...
    for n,c in enumerate(cues):
        ...
        if matches:
            ...
            evidence.append(dict(cue=n,start=c['start'],end=c['end'],
                                 error=error,audio_start=first,audio_end=last,phrase=phrase))
    return evidence

def interval_check(cues, words):
    evidence = phrase_evidence(cues, words)
    good = sum(x['error'] <= .75 for x in evidence)
    return dict(matched_cues=len(evidence),good=good,
                ratio=good/len(evidence) if evidence else 0,
                median_error=statistics.median(x['error'] for x in evidence) if evidence else None,
                passed=len(evidence)>=3 and good/len(evidence)>=.8,
                evidence=evidence)
...
def full_check(cues, words, duration):
    ...
    for i in range(10):
        ...
        ratio=sum(x['error']<=.75 for x in items)/len(items) if items else 0
        bins.append(dict(bin=i,matched=len(items),ratio=ratio,passed=len(items)>=5 and ratio>=.8))
    span=max((x['start'] for x in evidence),default=0)-min((x['start'] for x in evidence),default=0)
    coverage=len(evidence)/len(cues) if cues else 0
    valid=all(0<=c['start']<c['end']<=duration+2 and c['end']-c['start']<=15 for c in cues)
    passed=(valid and len(evidence)>=100 and coverage>=.6 and span>=duration*.8
            and all(x['passed'] for x in bins))
    return dict(passed=passed,valid_intervals=valid,matched=len(evidence),cue_coverage=coverage,
                span=span,bins=bins)
