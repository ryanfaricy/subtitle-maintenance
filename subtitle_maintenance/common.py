...
def run(command, timeout=120):
    ...
    try:
        ...
        raise
    if p.returncode:
        raise RuntimeError(f"{Path(str(command[0])).name} exited {p.returncode}: {err[-600:]}")
    return out
...
def install(source, target, root, expected, video, video_fp):
    ...
    if fingerprint(video) != video_fp:
        raise ValueError("Video changed during verification")
    current = digest(target) if target.exists() else None
    if current != expected:
        raise ValueError("Subtitle changed during verification")
    ...
    fd, name = tempfile.mkstemp(
        prefix="." + target.stem + ".", suffix=".staging", dir=target.parent
    )
    temp = Path(name)
    try:
        ...
        if (digest(target) if target.exists() else None) != expected or fingerprint(
            video
        ) != video_fp:
