...
def forced(stream):
    return bool(stream.get("disposition", {}).get("forced")) or bool(
        re.search(r"\b(forced|signs|songs)\b", stream.get("tags", {}).get("title", ""), re.I)
    )
...
def single_untagged_audio(data):
    ...
    s = tracks[0]
    tags = s.get("tags", {})
    if str(tags.get("language", "")).strip().lower() not in {"", "und"}:
        return None
    if auxiliary_audio(s):
        return None
    return s["index"]
...
def auxiliary_audio(stream):
    ...
    return any(flags.get(k) for k in ("comment", "visual_impaired", "descriptions")) or bool(
        re.search(
            r"commentary|description|descriptive", stream.get("tags", {}).get("title", ""), re.I
        )
    )
...
def choose_audio(streams):
    ...
    audio = [
        s
        for s in streams
        if s.get("codec_type") == "audio" and english_stream(s) and not auxiliary_audio(s)
    ]
...
def inventory(video):
    ...
    def full(stream):
        return english_stream(stream) and not forced(stream)
...
def sidecars(video):
    ...
    for p in sorted(video.parent.iterdir()):
        if not p.is_file() or p.is_symlink() or p.suffix.lower() not in SIDECAR:
            continue
        ...
        if set(tags) & {"forced", "backup", "old", "tmp", "temp", "corrected", "staging"}:
            continue
        if any(english(t) for t in tags):
            found.append(p)
...
def videos(paths, state_dir):
    ...
    for raw in paths:
        ...
        for p in [root] if root.is_file() else walk():
            if p.is_symlink() or not p.is_file() or p.suffix.lower() not in VIDEO:
                continue
            resolved = p.resolve()
            if state_dir == resolved or state_dir in resolved.parents:
                continue
            if any(
                part.startswith(".")
                for part in p.relative_to(root.parent if root.is_file() else root).parts
            ):
                continue
