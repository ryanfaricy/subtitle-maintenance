...
def resolve(identity, catalog):
    ...
    if not title.strip():
        raise MappingReview("No episode title available; numeric guessing disabled")
    ...
    if len(matches) != 1:
        raise MappingReview(f"Episode title has {len(matches)} exact catalog matches")
    e = matches[0]
    ...
    evidence = dict(
        ...
        url=e.get("url"),
        original_season=identity["season"],
        original_episode=identity["episode"],
        mapped_season=e["season"],
        mapped_episode=e["number"],
    )
...
def map_identity(identity, state):
    ...
    if not re.fullmatch(r"tt\d+", imdb):
        raise MappingReview("Invalid show IMDb ID")
    ...
    if cache.exists() and time.time() - cache.stat().st_mtime < 7 * 86400:
        ...
        def get(path):
            req = urllib.request.Request(
                "https://api.tvmaze.com/" + path, headers={"User-Agent": "SubtitleMaintenance/0.1"}
            )
...
def release_title_match(release, wanted, titles):
    ...
    if target and target in prefixes:
        return "match"
    ...
    title_part = re.split(
        r"(?i)(?:^|[\s._-])(?:\d{3,4}p|web(?:rip|[ ._-]?dl)?|hdtv|dvdrip|bluray|bdrip|x26[45]|h26[45]|aac|dd5)[\s._-]?",
        raw,
        maxsplit=1,
    )[0]
    ...
    if (
        ...
        and SequenceMatcher(None, target, candidate, autojunk=False).ratio() >= 0.92
    ):
        return "near_match"
    return "unknown"
...
def merge_candidates(groups, wanted, titles):
    ...
    for label, entries in groups:
        for entry in entries:
            ...
            if match == "different":
                rejected.append(
                    dict(entry, search_numbering=label, reason="Different episode title in release")
                )
                continue
            item = dict(entry, title_match=match, search_numberings=[label])
            old = by_id.get(entry["file_id"])
            if old:
                if label not in old["search_numberings"]:
                    old["search_numberings"].append(label)
                rank = {"match": 0, "near_match": 1, "unknown": 2}
                if rank[match] < rank[old["title_match"]]:
                    old["title_match"] = match
            else:
                by_id[entry["file_id"]] = item
    # Stable sort keeps provider ordering (including XL preference) within tiers.
    return sorted(
        by_id.values(), key=lambda e: {"match": 0, "near_match": 1, "unknown": 2}[e["title_match"]]
    ), rejected
...
def search_both(provider, original, mapped, state, prefer_xl):
    ...
    for label, ident in [("library", original), ("mapped", mapped)]:
        ...
        seen.add(key)
        try:
            groups.append((label, provider.candidates(ident, prefer_xl)))
        except Exception as error:
            errors.append(dict(numbering=label, error=str(error)))
    if not groups:
        raise RuntimeError("Both episode searches failed: " + str(errors))
