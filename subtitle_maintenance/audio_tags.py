...
def tag_missing(video, apply, state, config):
    ...
    if index is None:
        return dict(
            result,
            status="NO_SAFE_AUDIO_TAG_CHANGE",
            detail="Requires exactly one untagged non-commentary audio stream",
        )
    if video.suffix.lower() != ".mkv":
        return dict(
            result,
            status="REVIEW_CONTAINER",
            detail="Persistent audio tagging supports MKV only; use the assumption option for other formats",
        )
    ...
    st = video.stat()
    if (
        shutil.disk_usage(video.parent).free < 2 * st.st_size + 64 * 1024 * 1024
        or shutil.disk_usage(state).free < 2 * st.st_size + 64 * 1024 * 1024
    ):
    ...
    with tempfile.TemporaryDirectory(prefix=".audio-tag-", dir=video.parent) as d:
        ...
        if digest(copy) != sha:
            raise ValueError("Staging copy checksum mismatch")
        ...
        before_streams = info["data"]["streams"]
        after_streams = after["data"]["streams"]
        ...
        if (
            list(map(signature, before_streams)) != list(map(signature, after_streams))
            or after["audio_index"] != index
            or abs(after["duration"] - info["duration"]) > 0.1
        ):
