...
def inspect(video, mode, apply, state, config=None):
    ...
    result = dict(
        video=str(video),
        tracks=tracks,
        english_sidecars=list(map(str, sidecars)),
        unlabelled_sidecars=list(map(str, unknown)),
    )
    if mode == "coverage":
        status = (
            "EMBEDDED_ENGLISH_TEXT"
            if inv["text"]
            else "ENGLISH_SIDECAR"
            if sidecars
            else "ENGLISH_BITMAP_ONLY"
            if inv["bitmap"]
            else "NO_CONFIRMED_ENGLISH_TEXT"
        )
    elif mode == "defaults":
        defaults = [s for s in tracks if s.get("disposition", {}).get("default")]
        good = [s for s in inv["text"] if s.get("disposition", {}).get("default")]
        status = (
            "MULTIPLE_SUBTITLE_DEFAULTS"
            if len(defaults) > 1
            else "DEFAULT_ENGLISH_TEXT"
            if good
            else "NO_DEFAULT_FULL_ENGLISH_TEXT"
        )
    else:
