...
def transcript(video,info,start,length,config,state):
    model=config['model'];audio=info['audio_index']
    if audio is None:raise ValueError('English audio is unknown or ambiguous; select an audio stream explicitly')
    ...
    if legacy and audio_streams and audio==audio_streams[0]['index']:
        st=video.stat();oldfp=hashlib.sha256(f'{video.resolve()}\0{st.st_size}\0{st.st_mtime_ns}'.encode()).hexdigest()
        oldkey=hashlib.sha256(f'{oldfp}:{start}:{length}:{model}:native-word-v1'.encode()).hexdigest()
        old=Path(legacy)/(oldkey+'.json')
        if old.exists():return json.loads(old.read_text())
    if config.get('cache_only'):raise ValueError('Native transcript not cached; rerun without --cache-only')
    if not config.get('python') or not model:raise ValueError('Whisper Python/model not configured')
    ...
    with lockpath.open('a') as lock:
        ...
        while True:
            if time.monotonic()>deadline:raise RuntimeError('Whisper busy; retry later')
            if idle():
                try:
        ...
        try:
            with tempfile.TemporaryDirectory(prefix='subtitle-audio-') as d:
                audiofile=Path(d)/'audio.wav';output=Path(d)/'words.json'
                cmd=['ffmpeg','-nostdin','-v','error','-threads','1','-i',video]
                if start is not None:cmd+=['-ss',str(start),'-t',str(length)]
                cmd+=['-map',f'0:{audio}','-vn','-ac','1','-ar','16000','-y',audiofile]
                print('    Transcribing '+('full episode' if start is None else f'{start}s sample'),flush=True)
                run(cmd,600)
                import wave
                with wave.open(str(audiofile)) as w:actual=w.getnframes()/w.getframerate()
                expected=length if length else info['duration']
                if actual<expected*.98:raise ValueError('Incomplete decoded audio')
                worker=Path(__file__).with_name('native_worker.py')
                run([config['python'],worker,audiofile,output,model],3600)
                data=json.loads(output.read_text());atomic_json(cache,data)
                if fingerprint(video)!=fp:raise ValueError('Video changed during transcription')
                return data
