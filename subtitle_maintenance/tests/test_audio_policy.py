...
from subtitle_maintenance.providers import DownloadBudgetReached, Provider
...
class AudioPolicyTests(unittest.TestCase):
    def stream(self, language=None, title=""):
        return dict(
            index=1,
            codec_type="audio",
            tags=dict(title=title, **({"language": language} if language is not None else {})),
        )
    ...
    def test_tag_preview_and_container_gate(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            with (
                patch(
                    "subtitle_maintenance.media.inventory",
                    return_value={"data": {"streams": [self.stream()]}},
                ),
                patch("subtitle_maintenance.audio_tags.run") as run,
            ):
                self.assertEqual(
                    tag_missing(p, False, Path(d), {"min_age_minutes": 0})["status"],
                    "WOULD_TAG_AUDIO_ENGLISH",
                )
    ...
    def test_budget_is_typed_and_cache_exempt(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            p.state = Path(d)
            p.config = {"max_downloads": 0}
            p.downloads = 0
            with self.assertRaises(DownloadBudgetReached):
                p.download({"file_id": 1})
            (Path(d) / "downloads").mkdir()
            (Path(d) / "downloads/1.srt").write_text("cached")
