...
class MaintenanceTests(unittest.TestCase):
    ...
    def test_srt_render_preserves_text(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertEqual(
                [c["text"] for c in subtitles.read(source)],
                [c["text"] for c in subtitles.read(out)],
            )
    ...
    def fixture(self):
        ...
        for i in range(120):
            ...
            words.extend((t, i * 10 + 1.5 + j * 0.5) for j, t in enumerate(tokens))
        return cues, words
    ...
    def test_natural_lead_passes_and_wrong_cut_fails(self):
        ...
        self.assertFalse(
            full_check(cues, [(w, t + 40 if t > 600 else t) for w, t in words], 1200)["passed"]
        )
    ...
    def test_backup_install_and_restore(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            receipt = common.install(
                source,
                target,
                d / "backups",
                common.digest(target),
                video,
                common.fingerprint(video),
            )
            self.assertEqual(target.read_bytes(), b"new")
            self.assertEqual(Path(receipt["backup"]).read_bytes(), b"old")
            common.install(
                Path(receipt["backup"]),
                target,
                d / "rollback",
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            with patch.object(media, "inventory", return_value=info):
                result = workflow.process(
                    v,
                    SimpleNamespace(audio_stream=None, scan_only=True),
                    {"min_age_minutes": 0},
                    Path(d),
                    provider,
                )
    ...
    def test_new_sidecar_does_not_overwrite_unexpected_file(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            with self.assertRaises(ValueError):
                common.install(source, target, d / "b", None, v, common.fingerprint(v))
