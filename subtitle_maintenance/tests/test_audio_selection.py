...
class AudioSelectionTests(unittest.TestCase):
    def track(self,index,channels=2,default=0,language='eng',title='',**flags):
        return dict(index=index,channels=channels,codec_type='audio',
                    tags=dict(language=language,title=title),disposition=dict(default=default,**flags))
    ...
    def test_non_dialogue_and_non_english_excluded(self):
        for bad in [self.track(1,default=1,title='Commentary'),self.track(1,default=1,comment=1),
                    self.track(1,default=1,visual_impaired=1),self.track(1,default=1,descriptions=1),
                    self.track(1,default=1,language='fra'),self.track(1,language='und')]:
    ...
    def test_untagged_commentary_flag_refused(self):
        self.assertIsNone(single_untagged_audio({'streams':[self.track(1,language='und',comment=1)]}))
