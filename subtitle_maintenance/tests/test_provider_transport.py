...
from subtitle_maintenance.providers import (
    Provider,
    ProviderTransportError,
    legacy_transport_cooldown,
    provider_error_label,
    read_helper_response,
)
...
class ProviderTransportTests(unittest.TestCase):
                ...
                try:
                    ...
                    if expected:
                        with self.assertRaisesRegex(ProviderTransportError, expected):
                            read_helper_response(process, 0.01)
                    else:
                        self.assertEqual(read_helper_response(process, 0.01), {"data": []})
                finally:
                    os.close(write)
        read, write = os.pipe()
        os.close(write)
    ...
    def test_search_retries_once(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            with (
                patch.object(
                    p, "exchange", side_effect=[ProviderTransportError("helper_eof"), {"data": []}]
                ) as exchange,
                patch.object(p, "close") as close,
            ):
    ...
    def test_download_never_replayed_and_diagnostics_saved(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            with (
                patch.object(
                    p, "exchange", side_effect=BrokenPipeError("secret not to log")
                ) as exchange,
                patch.object(p, "close"),
            ):
                with self.assertRaisesRegex(RuntimeError, "BrokenPipeError"):
                    p.request({"action": "download"})
    ...
    def test_search_retry_bounded_and_legacy_narrow(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            with (
                patch.object(
                    p, "exchange", side_effect=ProviderTransportError("response_timeout")
                ) as exchange,
                patch.object(p, "close"),
            ):
                ...
                self.assertEqual(exchange.call_count, 2)
        self.assertTrue(
            legacy_transport_cooldown({"error": {"error": "Provider transport failure"}})
        )
        self.assertFalse(
            legacy_transport_cooldown({"error": {"error": "HTTPError", "status": 429}})
        )
