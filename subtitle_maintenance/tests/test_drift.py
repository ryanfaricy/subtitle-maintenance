...
class DriftTests(unittest.TestCase):
    def test_end_to_end_synthetic_dialogue_and_render(self):
        ...
        for i, y in enumerate(range(10, 1990, 4), 1):
            ...
            words.extend((token, y - 0.6 + j * 0.4) for j, token in enumerate(tokens))
            start = (y - 1) / 1.042 - 1
            end = (y - 1) / 1.042 + 1
            blocks.append(
        ...
        with tempfile.TemporaryDirectory() as folder:
            ...
            with (
                patch.object(workflow.native, "transcript", side_effect=transcript),
                patch.object(workflow.native, "words", side_effect=lambda data, *args: data),
            ):
                candidate, check = workflow.prepare(
                    Path("S03E02.mkv"),
                    source,
                    {"duration": 2000},
            ...
            self.assertEqual(
                subtitles.read(source)[0]["text"], subtitles.read(candidate)[0]["text"]
            )

    def evidence(self, scale=1.042, offset=1):
        return [
            dict(
                start=(y - offset) / scale - 1,
                end=(y - offset) / scale + 1,
                audio_start=y - 1,
                audio_end=y + 1,
            )
            for y in range(10, 1990, 4)
        ]
    ...
    def test_rejects_sparse_extreme_and_piecewise(self):
        ...
        for evidence in [
            self.evidence()[:20],
            self.evidence(1.1),
            self.evidence(offset=80),
            broken,
        ]:
            with (
                self.subTest(evidence=evidence[:1]),
                patch.object(drift, "phrase_evidence", return_value=evidence),
            ):
    ...
    def test_disabled_and_combined_do_not_fit(self):
        for name, config in [("S03E01", {}), ("S03E08-E09", {"allow_drift_correction": True})]:
            with (
                patch.object(subtitles, "read", return_value=[]),
                patch.object(workflow.native, "transcript"),
                patch.object(workflow.native, "words", return_value=[]),
                patch.object(
                    subtitles, "validate", return_value={"passed": False, "full": {"passed": False}}
                ),
                patch.object(drift, "fit") as fit,
            ):
    ...
    def test_render_rechecks_five_samples_with_no_shift(self):
        ...
        with (
            ...
            patch.object(workflow.native, "transcript"),
            patch.object(workflow.native, "words", return_value=[]),
            patch.object(subtitles, "validate", side_effect=validate),
        ):
            workflow.verify(
                Path("S03E01"),
                Path("test.srt"),
    ...
    def test_prepare_routes_scale_only_to_five_sample_recheck(self):
        with tempfile.TemporaryDirectory() as folder:
            source = Path(folder) / "test.srt"
            source.write_text("1\n00:00:10,000 --> 00:00:12,000\nHello there!\n")
            with patch.object(
                workflow,
                "verify",
                side_effect=[
                    {"passed": True, "offset": 0, "scale": 1.04},
                    {"passed": True, "offset": 0},
                ],
            ) as verify:
                candidate, check = workflow.prepare(
                    Path("v.mkv"), source, {}, {}, Path(folder), Path(folder)
                )
