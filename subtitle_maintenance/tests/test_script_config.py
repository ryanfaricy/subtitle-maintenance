...
class ConfigurationTests(unittest.TestCase):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            with patch.object(settings, 'REPO_ROOT', root), patch.dict(os.environ, {}, clear=True):
                ...
                with patch.dict(os.environ, {settings.CONFIG_ENV: str(explicit)}):
                    ...
                    self.assertEqual(settings.load_config(default)['media_root'], str(root / 'library'))
                default.unlink()
    ...
    def test_tool_override_and_show_scope(self):
        ...
        self.assertEqual(Path(settings.tool(config, 'ffmpeg')).resolve(), Path(sys.executable).resolve())
        self.assertEqual(settings.show_path(config, 'QI'), Path('/example/media/TV/QI'))
        with self.assertRaises(ValueError):settings.show_path({}, 'QI')
        with self.assertRaises(ValueError):settings.tool({'tools': {'ffmpeg': '/missing/tool'}}, 'ffmpeg')
    ...
    def test_main_cli_config_root_and_state_without_media(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            result = subprocess.run([sys.executable, str(ROOT / 'subtitle-maintain.py'), '--scan-only'],
                                    cwd=d, env=env, text=True, capture_output=True)
            self.assertEqual(result.returncode, 0, result.stderr)
            self.assertTrue((root / 'state/state.db').exists())
            (root / 'override-input').mkdir()
            result = subprocess.run([sys.executable, str(ROOT / 'subtitle-maintain.py'), str(root / 'override-input'),
                                     '--scan-only', '--state-dir', str(root / 'override-state')],
                                    cwd=d, env=env, text=True, capture_output=True)
            self.assertEqual(result.returncode, 0, result.stderr)
            self.assertTrue((root / 'override-state/state.db').exists())
            result = subprocess.run([sys.executable, str(ROOT / 'subtitle-maintain.py'), '--scan-only',
                                     '--config', str(root / 'missing.json')], env=env, text=True, capture_output=True)
    ...
    def test_whisper_cli_overrides_before_any_work(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            root = Path(d).resolve()
            config = root / 'config.json'
            config.write_text(json.dumps({'media_root': str(root / 'absent'),
                'whisper': {'model': 'configured', 'state_dir': './state'},
                'tools': {name: sys.executable for name in ('ffmpeg','ffprobe','curl')}}))
            with patch.object(sys, 'argv', ['whisper', '--config', str(config), '--model', 'override']), \
    ...
    def test_importer_reads_shared_url_without_loading_legacy_script(self):
        ...
        with patch.object(importer, 'load_config', return_value={'sonarr': {'url': 'http://example.test/api/v3'}}), \
             patch.object(importer, 'get_secret', return_value='fixture'), patch.dict(os.environ, {}, clear=True):
    ...
    def test_backup_command_keeps_paths_as_arguments(self):
        ...
        with patch.object(sys, 'argv', ['backup']), patch.object(backup, 'load_config', return_value=config), \
             patch.object(backup, 'tool', return_value='/fake/docker'), patch.object(backup.subprocess, 'run') as run:
    ...
    def test_common_command_uses_configured_tool(self):
        ...
        try:
            self.assertEqual(common.run(['ffprobe', '-c', 'print("configured")']).strip(), 'configured')
        finally:common.configure_tools({})
