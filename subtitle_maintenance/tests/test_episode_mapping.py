...
from subtitle_maintenance.episode_mapping import (
    MappingReview,
    merge_candidates,
    release_title_match,
    resolve,
...
class EpisodeMappingTests(unittest.TestCase):
    def setUp(self):
        self.identity = dict(
            imdb="tt0397306", season=10, episode=1, title="Steve and Snot's Test-Tubular Adventure"
        )
        self.catalog = dict(
            imdb="tt0397306",
            show_id=215,
            episodes=[dict(id=1, season=9, number=1, name="Steve & Snot’s Test-Tubular Adventure")],
        )
    ...
    def test_duplicate_and_missing_fail_closed(self):
        ...
        with self.assertRaises(MappingReview):
            resolve(self.identity, self.catalog)
    ...
    def test_wrong_show_and_unnumbered_fail(self):
        ...
        with self.assertRaises(MappingReview):
            resolve(self.identity, self.catalog)
        self.catalog["imdb"] = self.identity["imdb"]
        self.catalog["episodes"][0]["number"] = None
        with self.assertRaises(MappingReview):
            resolve(self.identity, self.catalog)
    ...
    def test_filter_rank_deduplicate(self):
        ...
        groups = [
            (
                "library",
                [
                    dict(file_id=1, release="American.Dad.S12E08.HDTV"),
    ...
    def test_unknown_release_not_rejected(self):
        for release in [
            "American.Dad.S11E08.HDTV.x264-KILLERS",
            "Unknown naming format",
            "American Dad S11E08 Unknown Title.WEB",
        ]:
            self.assertEqual(
                release_title_match(release, "Morning Mimosa", ["Stan-Dan Deliver"]), "unknown"
            )

    def test_longest_title_and_number_marker(self):
        self.assertEqual(
            release_title_match(
                "American Dad 12x08 Morning Mimosa.WEB", "Morning Mimosa", ["Morning Mimosa"]
            ),
            "match",
        )
        self.assertEqual(
            release_title_match(
                "Show S01E01 Long Title Part II.WEB",
                "Long Title Part I",
                ["Long Title Part I", "Long Title Part II"],
            ),
            "different",
        )
    ...
    def test_short_exact_title_promoted_not_prefix(self):
        self.assertEqual(
            release_title_match("American.Dad.S12E02.CIAPOW.1080p.WEB-DL", "CIAPOW", []), "match"
        )
        self.assertEqual(release_title_match("Show.S01E01.CIAPOW.WEB", "CIA", []), "unknown")
        self.assertEqual(
            release_title_match("Show.S01E01.CIAPOW.WEB", "Other Title", ["CIAPOW"]), "unknown"
        )
    ...
    def test_ranking_tiers(self):
        ...
        names = [
            "Show.S01E01.HDTV",
            "Show.S01E01.Steve.and.Snots.Testubular.Adventure.WEB-DL",
            "Show.S01E01.Steve.and.Snots.Test.Tubular.Adventure.WEB-DL",
        ]
        kept, rejected = merge_candidates(
            [("library", [dict(file_id=i, release=n) for i, n in enumerate(names)])], title, []
        )
