    ...
    for attempt in range(2):
        if TOKEN is None:
            ...
            login = call(
                ...
                "login",
                dict(username=config["username"], password=config["password"]),
            )
            TOKEN = login["token"]
...
def main(args):
    global TOKEN, HOST, STAGE, SESSION
    # Bazarr supplies optional dependencies here; importing this module is inert.
    if "/app/bazarr/bin/libs" not in sys.path:
    ...
    def call(host, path, payload=None):
        ...
        for _ in range(3):
            if r.status_code not in (301, 302, 307, 308) or payload is not None:
                break
            ...
            if (
                ...
                or parsed.path != "/api/v1/subtitles"
            ):
                break
            url = target
        ...
        if r.status_code >= 300:
            raise urllib.error.HTTPError(
                url, r.status_code, "Provider error", r.headers, io.BytesIO(r.content)
            )
        return r.json()
    ...
    if args["action"] == "search":
        ...
        if args.get("season") is not None:
            ...
            params.update(imdb_id=args["imdb"].removeprefix("tt"))
        result = call(
            "api.opensubtitles.com", "subtitles?" + urllib.parse.urlencode(sorted(params.items()))
        )
        ...
        if r.status_code >= 300:
            raise urllib.error.HTTPError(
                "content", r.status_code, "Content error", r.headers, io.BytesIO(r.content)
            )
        content = r.content
    ...
    for line in sys.stdin:
        try:
            ...
            if hasattr(e, "read"):
                ...
                if not titles:
                    try:
                        body = str(json.loads(raw).get("message", ""))[:300]
                    except ValueError:
                        body = "non-JSON response without title"
