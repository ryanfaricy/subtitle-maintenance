#!/usr/bin/env python3
"""Preview reconciliation of EastEnders DVR recordings with Sonarr.

Import-safe legacy utility. Changes require --apply; source cleanup also requires
--cleanup-redundant. Prefer the general importer for other series.
"""
...
def reconcile(config, *, apply=False, cleanup_redundant=False):
    """Run the legacy matching workflow only after explicit CLI invocation."""
    import requests
    ...
    def host_to_sonarr_path(host_path):
        ...
        return (Path(SONARR_EASTENDERS_PATH) / relative).as_posix()
    ...
    matches = [s for s in series if s.get("title", "").lower() == "eastenders"]

    if len(matches) != 1:
        print(f"ERROR: Expected exactly one EastEnders series, but found {len(matches)}.")

        for s in matches:
            print(f"  id={s.get('id')} title={s.get('title')} path={s.get('path')}")
    ...
    for path in HOST_EASTENDERS_PATH.rglob("*"):
        ...
        files_to_process.append((path, year, episode_number))

    if not files_to_process:
        print("No Plex-style EastEnders recordings found.")
        raise SystemExit(0)

    print(f"Found {len(files_to_process)} Plex-style recording(s).")
    print()
    ...
    for key, paths in grouped.items():
        preferred_file_for_key[key] = max(paths, key=lambda p: p.stat().st_size)
    ...
    for path, year, episode_number in sorted(
        ...
        if path != preferred_file:
            print(f"SKIP: Smaller duplicate for {year} episode {episode_number}")
            print(
                f"      Keeping: {preferred_file.name} "
                f"({preferred_file.stat().st_size / 1024 / 1024:.1f} MB)"
            )
            print(f"      Skipping: {path.name} ({path.stat().st_size / 1024 / 1024:.1f} MB)")
            print()
            continue

        if len(grouped[key]) > 1:
            print(f"DUPLICATE: Using largest file for {year} episode {episode_number}:")
        ...
        if episode is None:
            print(f"SKIP: Cannot uniquely match {year} episode {episode_number} in Sonarr.")
            print()
            continue
        ...
        if episode.get("hasFile"):
            print(f"SKIP: Sonarr already has S{season_number:02d}E{sonarr_episode_number:02d}")

            try:
                existing_episode = sonarr_get(f"/episode/{episode_id}")
                ...
                existing_relative = Path(existing_sonarr_path).relative_to(sonarr_root)
                ...
                if not cleanup_redundant:
                    print("      CLEANUP SKIP: use --cleanup-redundant to request deletion.")
                    continue
                if DRY_RUN:
                    print("      WOULD DELETE (preview only).")
                    continue
                path.unlink()
        ...
        print(f"MATCH: Plex S{year}E{episode_number}")

        print(f"       Sonarr S{season_number:02d}E{sonarr_episode_number:02d}")
        ...
        print(f"       Sonarr path: {sonarr_file_path}")
        ...
        try:
            ...
            manual_items = [item for item in manual_items if item.get("path") == sonarr_file_path]

        except requests.RequestException as exc:
            print(f"ERROR: Sonarr could not inspect this file: {exc}")
            print()
            continue
        ...
        if quality is None:
            print("SKIP: Sonarr did not return quality information.")
            print()
            continue
        ...
        if DRY_RUN:
            ...
            print("WOULD IMPORT:")
            print(f"       {path.name}")
            print(f"       as Sonarr S{season_number:02d}E{sonarr_episode_number:02d}")
            print(f"       using Sonarr episode ID {episode_id}")
            print()
            continue
        ...
        try:
            ...
            print()
            print("IMPORT SUBMITTED TO SONARR")
            ...
            for _ in range(30):
                ...
                if status == "failed":
                    print("ERROR: Sonarr import command failed.")
                    break
            else:
                print("ERROR: Timed out waiting for Sonarr import.")
                continue
            ...
            imported_episode = sonarr_get(f"/episode/{episode_id}")

            episode_file_id = imported_episode.get("episodeFileId")

            if not episode_file_id:
                print("ERROR: Import completed but Sonarr did not assign an episodeFileId.")
                continue
            ...
            print(f"       Rename submitted: {rename_command_id}")
            ...
            for _ in range(30):
                ...
                rename_command = sonarr_get(f"/command/{rename_command_id}")
            ...
            print()
            print(f"ERROR submitting import: {exc}")
    ...
    if DRY_RUN:
        print("DRY RUN COMPLETE. No files were imported or changed.")
    else:
        print("IMPORT REQUESTS SUBMITTED TO SONARR.")
...
def main(argv=None):
    ...
    parser.add_argument(
        "--preview", action="store_true", help="Explicitly request a read-only preview"
    )
    parser.add_argument("--apply", action="store_true", help="Allow imports and renames")
    parser.add_argument(
        "--cleanup-redundant",
        action="store_true",
        help="Also allow verified redundant-source cleanup with --apply",
    )
    ...
    if args.preview and args.apply:
        parser.error("--preview and --apply cannot be combined")
    try:
        config = load_config(args.config)
        show_path(config, "EastEnders")
        if not config.get("sonarr", {}).get("eastenders_root"):
            parser.error("Set sonarr.eastenders_root in config")
    except ValueError as exc:
        parser.error(str(exc))
    return reconcile(config, apply=args.apply, cleanup_redundant=args.cleanup_redundant)
